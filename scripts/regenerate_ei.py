...
def main():
    try:
        ...
        lines += ["", f"Command: `{command}`.",
            ...
            "", f"Per-window values: {out_text}/ei_xcheck.csv. {out_text}/ei_xcheck.json carries the pass's mask_meta.json as a verbatim copy under `metadata` (its `note` field is in Chinese and describes the mask export stage). Generated PNGs: {out_text}/ei_regen_check/ (versioned with this repository)."]
